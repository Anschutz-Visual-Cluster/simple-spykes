...
def graph_spikeinterface_quality_metrics_prob_dists(metrics_file: str, save_folder: Union[bool, str] = False, use_common_units: bool = False, save_prefix: str = ""):
    ...
    def graph_qm(qm_name: str, qm_data: list[float]):
        ...
        bin_edges = np.linspace(
            min_qm_value,
            max_qm_value,
            num=math.ceil(math.fabs(num_bins))  # Round up to include values that lie in part of a bin_size on the pos edge
        )
        ...
        plt.xlabel(f"{qm_name} value (binned by {round(bin_size, 2)})")
